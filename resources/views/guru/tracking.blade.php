...
<div class="page-header">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('guru.orders.show', $order) }}">#{{ $order->id }}</a></li>
            <li class="breadcrumb-item active">Live Tracking</li>
        </ol>
    </nav>
    <h1>Live Tracking Pengiriman #{{ $order->id }}</h1>
</div>

<!-- Info Bar -->
<div class="tracking-info-bar">
    <div class="d-flex align-items-center gap-3">
        @if($delivery && $delivery->tracking_active)
    ...
    <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <div class="status-chip" id="status-chip">
            <span id="status-text">{{ $delivery->status_label }}</span>
        </div>
        @endif
...
const trackingUrl = "{{ route('guru.tracking.data', $order) }}";
